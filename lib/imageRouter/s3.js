...
const { getImageExt } = require('../utils')
...
exports.uploadImage = function (imagePath, mimeType, callback) {
  ...
  fs.readFile(imagePath, function (err, buffer) {
    ...
    const fileExt = getImageExt(mimeType)
    if (fileExt) { params.Key += `.${fileExt}` }
